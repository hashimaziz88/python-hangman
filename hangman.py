...
def main():
    ...
    while count < 7:
        ...
        if letter_guess in correct_guess and letter_guess not in masked_word:
            ...
            if letter_guess in masked_word and correct_guess.count(letter_guess) == masked_word.count(letter_guess) :
                print(correct_guess.count(letter_guess))
                masked_word.count(letter_guess)
                print("This letter has already been revealed in the answer, Please eneter another guess. ")
                print(masked_word, f" Lives count: {count}. ")
            elif letter_guess in masked_word and correct_guess.count(letter_guess) > masked_word.count(letter_guess) :
                print("here")
                masked_word = update_mask(masked_word, random_word, letter_guess)
                print(masked_word, f" Lives count: {count}. ")
            else:
